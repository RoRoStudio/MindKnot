...
export const createPhysicsWorld = (width: number, height: number): PhysicsWorld => {
    ...
    const update = () => {
        // Apply repulsion between bodies - STRONGER FORCE TO PREVENT OVERLAPPING
        for (let i = 0; i < bodies.length; i++) {
            for (let j = i + 1; j < bodies.length; j++) {
                ...
                // If bodies are overlapping, push them apart with stronger force
                if (dist < minDist) {
                    ...
                    // Increased from 0.05 to 0.5 and made exponential based on closeness
                    const repulsionStrength = 0.5;
                    const force = repulsionStrength * Math.pow(minDist - dist, 1.5);
        ...
        for (const body of bodies) {
            ...
            // Apply stronger damping to velocity for more stable behavior
            body.velocity.x *= 0.8;
            body.velocity.y *= 0.8;
